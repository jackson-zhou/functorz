...
const runtimeSource = `import { Button, Image, Input, Swiper, SwiperItem, Text, View } from '@tarojs/components'
import Taro from '@tarojs/taro'
import project from './project.json'
type Node={id:string;type:string;props:Record<string,unknown>;style?:Record<string,unknown>;action?:{type:string;pageId?:string};children:Node[]}
...
function style(node:Node){const s=node.style??{};return {color:s.color as string,backgroundColor:s.backgroundColor as string,fontSize:s.fontSize?font[String(s.fontSize)]:undefined,padding:s.spacing?size[String(s.spacing)]:undefined,borderRadius:s.radius?radius[String(s.radius)]:undefined,gap:s.gap?size[String(s.gap)]:undefined,alignItems:s.align as string,gridTemplateColumns:s.columns?\`repeat(\${s.columns},minmax(0,1fr))\`:undefined}}
function action(node:Node){return async()=>{if(node.action?.type==='back')await Taro.navigateBack();if(node.action?.type==='navigate'){const page=project.pages.find(p=>p.id===node.action?.pageId);if(page)await Taro.navigateTo({url:page.route})}}}
function NodeView({node}:{node:Node}){const children=node.children.map(child=><NodeView node={child} key={child.id}/>);const props={style:style(node),onClick:action(node),className:\`fz-\${node.type.toLowerCase()}\`};if(node.type==='Text')return <Text {...props}>{String(node.props.text??'')}</Text>;if(node.type==='Image')return <Image {...props} src={String(node.props.src??'')} mode="aspectFill"/>;if(node.type==='Button')return <Button {...props}>{String(node.props.text??'')}</Button>;if(node.type==='Input')return <Input {...props} name={String(node.props.name??'')} placeholder={String(node.props.placeholder??'')}/>;if(node.type==='Swiper')return <Swiper {...props} autoplay={Boolean(node.props.autoplay)}>{node.children.map(child=><SwiperItem key={child.id}><NodeView node={child}/></SwiperItem>)}</Swiper>;return <View {...props}>{children}</View>}
export function GeneratedPage({pageId}:{pageId:string}){const page=project.pages.find(p=>p.id===pageId)??project.pages[0];return <NodeView node={page.root as Node}/>}
`
